...
#### Índice único (regra de negócio)

Garante que um usuário não tenha mais de uma conta do mesmo tipo:

```sql
CREATE UNIQUE INDEX ux_bank_accounts_user_type
ON bank_accounts (user_id, type);
```
